...
# 6. check if named.cache is present, if not download it (unbound needs it to resolve root servers)
if [ -f /app/named.cache ]; then
    info "named.cache already present — skipping download."
else
    info "Downloading named.cache..."
    curl -fsSL -o named.cache https://www.internic.net/domain/named.cache
fi
