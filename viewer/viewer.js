...
async function analyzeURL(url, wait = 2, scoreboard_entry = false, screenshot = 'none', ext = null, whois = 'true') {
  ...
  const progressInterval = setInterval(function() {
    ...
    const barValue = Math.min((Math.log1p(elapsed) / Math.log1p(serverMaxRetryTime)), 4*elapsed/serverMaxRetryTime); // logarithmic progress with fallback to linear after 50% time
