...
api_url = environ.get('WEBRES6_API_URL', 'https://test.webres.v6only.shoot.canary.k8s-hana.ondemand.com/res6')
...
def main():
    ...
    parser.add_argument("-o", "--save-json", type=argparse.FileType('w', encoding='utf-8'), metavar="FILE.json", help="Save JSON output to file")
    ...
    if args.save_json:
        json.dump(res, args.save_json)
        print(f"JSON dump saved to: {args.save_json.name}", file=sys.stderr)
    ...
    if not args.quiet:
        ...
        if args.display_screenshot and res.get('screenshot'):
            ...
            print("Got screenshot - you may use '-o >(jq -r | base64 -d > screenshot.png)' to save the image", file=sys.stderr)
        ...
        if ipv6_only_ready:
            ...
            print(f"Conclusion: {color['fail']}NOT THERE YET{color['reset']} - At least one host has only IPv4 address.")
